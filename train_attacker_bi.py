...
def main(args):
    ...
    for batch in trange(config['whole-batch-number']):# progress bar with training process
        ...
        for batch in trange(config['attacker-num-batches']):# progress bar with training process
            ...
            ##Adv_lr = Adv_lr/math.log(epoch+1)

            # if  grads[0] > max_value:
            #     grads[0] = max_value
            # elif grads[0] < min_value:
            #     grads[0] = min_value

            # Adv = Adv -  Adv_lr * grads[0]
            ...
            if stop_trigger >= config['stop-threshold']:
                break
      # lr decay
        if(epoch*epoch_big%config['lr_decay']==0):
            optimizer.param_groups[0]['lr']=config['attacker-lr']/((epoch*epoch_big/config['lr_decay'])**0.5)
    #----------------------------------------------------------------------------------
        epoch = 1.0
